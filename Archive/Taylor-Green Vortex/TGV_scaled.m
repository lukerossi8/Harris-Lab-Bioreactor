...
x0 = [0.01, 0.01; 0.01, 0.01; 0.01, 0.01]; % Initial position, m
...
A_TG = 0.001; % Amplitude of the TG vortex, in m^2/s
L_TG = 0.1; % Length of the TG vortex, in m
psi_TG = @(x,y) -A_TG*cos(pi*x/L_TG)*cos(pi*y/L_TG);

vfx = @(x,y) (A_TG * pi / L_TG) * cos(pi * x / L_TG) * sin(pi * y / L_TG);
vfy = @(x,y) -(A_TG * pi / L_TG) * sin(pi * x / L_TG) * cos(pi * y / L_TG);
