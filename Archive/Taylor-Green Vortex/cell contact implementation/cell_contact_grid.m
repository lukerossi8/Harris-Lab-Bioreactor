...
%% Defining agents
n_agents = 100; % number of agents modeled
...
% x0 = [0.0141808619364564; 0.0283736858126425; 0.0143208619364564; 0.0285136858126425; 0.0450888670849368]; % Initial x position, m
% y0 = [-0.0249688151425399; -0.0161331764886680; -0.0249688151425399; -0.0161331764886680; -0.0245834182492018]; % Initial y position, m
...
contact_grid = cell(101, 201); % Empty grid, to be populated with the cells that are in each grid square
% grid side length = 1e-3 m (comp. w/min. grid side length of 2.52e-4 m)

% Calculating which grid square each agent is in
grid_pts_x = round(x, 3); % To the nearest 0.001
grid_pts_y = round(y, 3);
grid_pts = [grid_pts_x, grid_pts_y];

for k=1:size(grid_pts, 1)
    point = grid_pts(k, :);
    x_norm = point(1)*1000 + 51; % mapping x and y coordinates to row & column indices in the matrix
    y_norm = point(2)*(-1000) + 51;
    grid_square = contact_grid{y_norm, x_norm};
    %adding the agents to the appropriate cell in the cell array
    if isempty(grid_square)
        contact_grid{y_norm, x_norm} = k; % i represents an agent (each row in grid_pts is an agent)
    else
        contact_grid{y_norm, x_norm}(end+1, :) = k;
    end
end

% Iterating through each grid square

for i=1:size(contact_grid, 1)
    for j=1:size(contact_grid, 2) % looping through each grid square
        grid_square = contact_grid{i,j};
        if size(grid_square, 1) > 1
            pairs = nchoosek(grid_square, 2); % all possible pairs of agents in that grid square
            % To improve this and catch edge cases, add pairs between each
            % agent in THAT grid square and all agents in adjacent grid squares
            % (but not between two agents in adjacent grid squares, that will
            % be dealt with later in the iteration). Do this by associating
            % each agent w/a grid square, then checking that grid square.
            % This avoids checking empty grid squares
