...
[X_grid,Y_grid] = meshgrid(l_wall : grid_sl : r_wall, floor : grid_sl : ceil);
psi_TG = -A_TG*cos(pi * X_grid / L_TG) .* cos(pi * Y_grid / L_TG);

% Define a discrete background velocity field (x and y cpts)
vfx_disc = (A_TG * pi / L_TG) * cos(pi * X_grid / L_TG) .* sin(pi * Y_grid / L_TG);
vfy_disc = -(A_TG * pi / L_TG) * sin(pi * X_grid / L_TG) .* cos(pi * Y_grid / L_TG);
...
% Initializing not near the edges or the 

l_count = randi([0, n_agents]);
r_count = n_agents - l_count;

x0_l = l_wall + 0.05*(r_wall - l_wall) + 0.4*(r_wall - l_wall).*rand(l_count, 1);
x0_r = l_wall + 0.55*(r_wall - l_wall) + 0.4*(r_wall - l_wall).*rand(r_count, 1);
x0 = [x0_l; x0_r];
...
odeFun = @(t, z) eom(t, z, mu, r_i, vfx_disc, vfy_disc, X_grid, Y_grid, m, gx, gy, rho_f, rho_i, n_agents);
...
%% Numerical solution
function dzdt = eom(t, z, mu, r_i, vfx_disc, vfy_disc, X_grid, Y_grid, m, gx, gy, rho_f, rho_i, n_agents)
...
% Matrices relating pairs of agents
