...
for i=1:n_microcarriers
    plot(z_all(:, i), z_all(:, i+n_microcarriers), 'color', "black", "LineWidth", 1.5);
    plot(s0(i), s0(i+n_microcarriers), "or", "LineWidth", 2)
end
grid on
xlim([-0.05 0.15])
ylim([-0.05 0.05])
...
xlabel('x position (m)')
ylabel('y position (m)')
title('Microcarrier trajectories in counterflow TG vortex')
