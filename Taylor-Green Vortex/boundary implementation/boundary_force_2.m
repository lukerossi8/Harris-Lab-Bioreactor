...
function dzdt = eom(t, z, mu, r_i, vfx_disc, vfy_disc, X, Y, m, gx, gy, rho_f, rho_i, n_microcarriers, l_wall, r_wall, floor, ceil)
    ...
    % Define a restitution coefficient for boundary collisions
    restitution_coefficient = 0.9;  % A value less than 1 simulates energy loss

    % ** Left and Right Boundary Reflections **
    % Apply spring-like force (restitution) for particles outside the left or right boundaries
    bound_left = x < l_wall;
    bound_right = x > r_wall;
    
    % If the particle is outside the left boundary, apply the restitution force
    if any(bound_left)
        vx(bound_left) = vx(bound_left) * -restitution_coefficient;  % Reverse and damp velocity
        x(bound_left) = l_wall;  % Set particle back to the boundary position
    end
    
    % If the particle is outside the right boundary, apply the restitution force
    if any(bound_right)
        vx(bound_right) = vx(bound_right) * -restitution_coefficient;  % Reverse and damp velocity
        x(bound_right) = r_wall;  % Set particle back to the boundary position
    end
    
    % ** Floor and Ceiling Boundary Reflections **
    % Apply spring-like force (restitution) for particles outside the floor or ceiling boundaries
    bound_floor = y < floor;
    bound_ceil = y > ceil;
    
    % If the particle is below the floor, apply the restitution force
    if any(bound_floor)
        vy(bound_floor) = vy(bound_floor) * -restitution_coefficient;  % Reverse and damp velocity
        y(bound_floor) = floor;  % Set particle back to the boundary position
    end
    
    % If the particle is above the ceiling, apply the restitution force
    if any(bound_ceil)
        vy(bound_ceil) = vy(bound_ceil) * -restitution_coefficient;  % Reverse and damp velocity
        y(bound_ceil) = ceil;  % Set particle back to the boundary position
    end
