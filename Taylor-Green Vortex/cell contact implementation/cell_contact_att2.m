...
x0 = [0.09; 0.0902]; % Initial x position, m
...
% This loop implements boundary interaction logic
while t(end) < tstop
    delta_t = 0.05; % Maximum time for one call to ode45
    tspan = [t_current, t_current + delta_t];  % Fixed step window (e.g. 0.05 s)
...
figure
hold on
plot(t_plot, dist_plot, 'LineWidth',2)
yline(2*r_i(1), '-', 'bonding threshold', 'LineWidth',1)
grid on
xlabel('time (s)')
ylabel('particle distance (m)')
title('distance between particles 1 and 2 — without bonding')
end