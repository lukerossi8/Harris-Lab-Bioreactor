...
x0 = [-0.01; 0.01]; % Initial x position, m
y0 = [-0.01; -0.01]; % Initial y position, m
s0 = [x0; y0]; % Initial positions, m

vx0 = [0.0001; -0.0001]; % Initial x velocity, m/s
vy0 = [0.0001; 0.0001]; % Initial y velocity, m/s
v0 = [vx0; vy0]; % Initial velocity, m/s

% Properly format initial conditions for ode45
z0 = [s0; v0];

% Initial velocity values were at one point inspired by range seen in CMMC paper, Fig. 5A
...
% This loop implements cell bonding logic and boundary interaction logic
while t(end) < tstop
    delta_t = 0.05; % Maximum time for one call to ode45
    tspan = [t_current, min(t_current + delta_t, tstop)];  % Short time window for ode45 call
    ...
    [t, z_all] = ode45(odeFun, tspan, z_current, options);

    % Check for bonding; update list
    ...
    bond = ismember(bonded_pairs, [1, 2], "rows")|ismember(bonded_pairs, [2,1], "rows");
    if any(bond) % If the particles are bonded
            if dist > delta_d % If the bond is broken
                ...
                % Else, don't change bonded_pairs (solve with the bond force (plus drag and buoyancy)
            end
        else
            if dist < delta_c
                ...
                % Else, solve without the bond force
            end
        end
    ...
    tspan = [t(end), min(t_current + delta_t, tstop)];
...
xlabel('x position (m)')
ylabel('y position (m)')
title('Cell-cell bonding proof-of-concept, background flows and other forces off')
...
% Fully vectorized numerical solution
    function dzdt = eom(t, z, mu, r_i, vfx_disc, vfy_disc, X, Y, m, gx, gy, rho_f, rho_i, n_microcarriers, bonded_pairs)
        ...
        theta_1 = atan2(-dist_y, -dist_x); % Angle of particle 1 relative to particle 2, radians
        theta_2 = atan2(dist_y, dist_x); % Angle of particle 2 relative to particle 1, radians
...
grid on
xlabel('time (s)')
ylabel('distance between particles 1 and 2')
end


% Still left to do:
% - Difference between cell-cell bond and cell-microcarrier bond
% - Difference between cells and microcarriers, period
% - Expand bonding implementation to handle multiple particles
% - Determine the microcarrier mass (thus density), so we can meaningfully
%   implement the 200 µm microcarrier radius