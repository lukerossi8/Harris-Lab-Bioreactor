%% NOTE TO SELF: Next steps after Wed Aug 20
% The script handles an arbitrary number of bonds among an arbitrary number
% of agents. The next steps will be to speed up the program by implementing
% a grid system for checking for possible bonds, and then distinguishing
% between cells and microcarriers. Both of these are making me think an
% object-oriented approach would be better (I could make classes for cells
% and microcarriers, so that each agent is literally an object in the
% program). Worth asking about on Friday.
...
% Calculating agent distances & updating bonded_pairs
agent_indices = (1:n_agents)'; % Column vector with elts 1 to n_agents
pairs = nchoosek(agent_indices, 2); % All possible pairs of agents
for i = 1:size(pairs, 1) % for each pair of agents in the simulation
